 ...
 * @author Paksas
 */
public class MovieScreen extends Screen implements MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, OnCompletionListener, InputHandler
{
	enum State
	{
		Initialize,
	...
	public void resume() 
	{
		...
		// change the state to kick off player initialization process
		synchronized( m_stateSynchObj )
		{
			m_currentState = State.InitializeGraphics;
	...
	public void onPrepared( MediaPlayer player ) 
	{
		...
		Vector3 viewportSize = new Vector3( m_viewportWidth, m_viewportHeight, 0 );
		Vector3 quadDimensions = new Vector3( width, height, 0 );
		MathLib.processAspectRatio( viewportSize, quadDimensions );
	
		// create the fullscreen quad now that we know the dimensions of the movie clip
		initializeFullscreenQuad( m_game.getGraphics(), quadDimensions.m_x, quadDimensions.m_y );
		
		// initialize rendering position
		m_posX = (float)( m_viewportWidth - quadDimensions.m_x ) * 0.5f; 
		m_posY = (float)( m_viewportHeight - quadDimensions.m_y ) * 0.5f; 
	...
	/**
	 * A helper method that cleans up once the movie's played. 
	 */
	...
	/**
	 * Draws the movie on the provided render surface.
	 */
	private void draw()
	{
		synchronized( m_synchObj )
